            ...
            <div>
                <div class="filtersDiv">
                    <div class="row filter-row">
                        <h5><?= $translator->translate('Price Range') ?></h5>
                        <div class="col-md-5">
                            <input class="filter-input" placeholder="<?= $translator->translate('From') ?>" v-model="filters.from"></ipnut>
                        </div>
                        <div class="col-md-4">
                            <input class="filter-input" placeholder="<?= $translator->translate('To') ?>" v-model="filters.to"></ipnut>
                        </div>
                        <div class="col-md-2">
                            <button class="priceFilterBtn" @click="priceFilter">></button>
                        </div>
                    </div>
                    <div class="row filter-row filterOptions">
                        <div class="col-md-12">
                            <button @click='toggleShow' class='anchor' v-if="selectedFilter == ''"><?= $translator->translate('Sort By')?></button>
                            <button @click='toggleShow' class='anchor' v-if="selectedFilter != ''">{{selectedFilter}}</button>
                            <div v-if='showMenu' class='menu'>
                                <div class='menu-item' v-for='item in this.items' @click='itemClicked(item)'>{{item}}</div>
                            </div>
                        </div>
                    </div>
                    <div class="row filter-row" v-if="error_check_price">
                        numbers
                    </div>
                </div>
